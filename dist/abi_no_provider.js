...
exports.generateContractClass = exports.generateTypeSerializers = exports.generateTypeInterfaces = void 0;
/**
 * Generates code for TypeScript interfaces of data types
 */
function generateTypeInterfaces(abi) {
    let output = '';
    const types = new Map();
    // First collect all types from messages
    for (const msg of abi.internal_messages) {
        if (!msg.definition_name)
            continue;
...
exports.generateTypeInterfaces = generateTypeInterfaces;
/**
 * Generates code for type serialization functions
 */
function generateTypeSerializers(abi) {
    let output = '';
    for (const msg of abi.internal_messages) {
        if (!msg.definition_name)
            continue;
...
exports.generateTypeSerializers = generateTypeSerializers;
/**
 * Generates code for contract class
 */
function generateContractClass(abi, contractName) {
    let output = '';
    // List of get-methods
...
exports.generateContractClass = generateContractClass;
// Helper functions
/**
 * Map TON type to TypeScript type
 */
function mapToTsType(tonType) {
    if (!tonType)
        return 'any';
 ...
 */
function generateStoreCode(param) {
    const tsType = mapToTsType(param.type);
    if (tsType === 'bigint') {
 ...
 */
function generateLoadCode(param) {
    const tsType = mapToTsType(param.type);
    if (tsType === 'bigint') {
 ...
 */
function generateTupleReadCode(param) {
    const tsType = mapToTsType(param.type);
    if (tsType === 'bigint') {
 ...
 */
function generateTupleWriteCode(param) {
    const tsType = mapToTsType(param.type);
    if (tsType === 'bigint') {
